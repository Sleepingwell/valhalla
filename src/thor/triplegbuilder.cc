...
void TripLegBuilder::Build(
  ...
  *trip_path.mutable_algorithms() = {algorithms.begin(), algorithms.end()};
  ...
  std::vector<uint64_t> trip_osmids;
  ...
  for (auto edge_itr = path_begin; edge_itr != path_end; ++edge_itr, ++edge_index) {
    ...
    bool have_osmids = edgeinfo.has_osmids();

    if(have_osmids) {
      assert(edgeinfo.osmids_size() == edge_shape_lls.size());
      auto const* osmids = edgeinfo.osmids();
      std::transform(edge_shape_lls.begin(), edge_shape_lls.end(), osmids, edge_shape.begin(),
    ...
    if (edge_trimming && !edge_trimming->empty() && edge_trimming->count(edge_index) > 0) {
      ...
      std::transform(edge_shape.begin(), edge_shape.end(), edge_shape_lls.begin(), [](PointLLAndOSMId const& lli) { return lli.first; } );

      // keep the shape
      trip_shape.insert(trip_shape.end(), edge_shape_lls.begin() + !edge_begin_info.trim, edge_shape_lls.end());
      ...
      // keep the shape
      trip_shape.insert(trip_shape.end(), edge_shape_lls.begin() + !is_first_edge, edge_shape_lls.end());
